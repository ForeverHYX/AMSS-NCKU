...
// ... (保留 GPU 相关代码不变) ...
    ...
    if (dim != 3)
    {
        cout << "distribute: now we only support 3-dimension" << endl;
    ...
    while (PLi)
    {
        ...
        if (lev != PLi->data->lev)
            cout << "Parallel::distribute CAUTION: meet Patches for different level: " << lev << " and " << PLi->data->lev << endl;
        PLi = PLi->next;
    }
    ...
    // ------------------------------------------------------------------------
    // [新映射函数] 手动查表切分 (Manual Table Split)
    // 专门针对 4 份切分 (16进程=4x4) 的场景
    // ------------------------------------------------------------------------
    auto get_manual_split = [&](int split_idx, int total_splits, int total_len) -> int {
        // 边界处理
        if (split_idx == 0) return 0;
        if (split_idx == total_splits) return total_len;
        
        // 计算归一化位置 (0.0 ~ 1.0)
        double ratio = 0.0;

        // 【情况 A】正好切 4 份 (最常见的情况，如 X 和 Y 轴)
        // 此时我们应用对称切分逻辑： [ ratio | 0.5 | 1.0-ratio ]
        if (total_splits == 4) 
        {
            if (split_idx == 1)      ratio = manual_cut_ratio;         // 第一刀
            else if (split_idx == 2) ratio = 0.5;                      // 中间一刀 (强制居中)
            else if (split_idx == 3) ratio = 1.0 - manual_cut_ratio;   // 第三刀 (对称)
        }
        // 【情况 B】正好切 2 份 (可能发生在 Z 轴)
        // 只有中间一刀
        else if (total_splits == 2)
        {
            ratio = 0.5;
        }
        // 【情况 C】其他切分数量 (退化为均匀切分)
        ...
        {
            ratio = (double)split_idx / total_splits;
        }

        // 映射回物理坐标
        int pos = (int)(ratio * total_len);

        // 安全性钳位
        return Mymax(0, Mymin(total_len, pos));
    };
    // ------------------------------------------------------------------------
        ...
        for (int i = 0; i < nxyz[0]; i++)
            for (int j = 0; j < nxyz[1]; j++)
                for (int k = 0; k < nxyz[2]; k++)
                {
                    ...
                    // X 轴
                    ibbox_here[0] = get_manual_split(i, nxyz[0], PP->shape[0]);
                    ibbox_here[3] = get_manual_split(i + 1, nxyz[0], PP->shape[0]) - 1;
                    
                    // Y 轴
                    ibbox_here[1] = get_manual_split(j, nxyz[1], PP->shape[1]);
                    ibbox_here[4] = get_manual_split(j + 1, nxyz[1], PP->shape[1]) - 1;
                    
                    // Z 轴
                    ibbox_here[2] = get_manual_split(k, nxyz[2], PP->shape[2]);
                    ibbox_here[5] = get_manual_split(k + 1, nxyz[2], PP->shape[2]) - 1;
...
// ... (保留)
    ...
    if (reacpu < nodes * 2 / 3)
    {
        ...
        if (myrank == 0)
            cout << "Parallel::distribute CAUTION: level#" << lev << " uses essencially " << reacpu << " processors vs " << nodes << " nodes run, your scientific computation scale is not as large as you estimate." << endl;
    }
