                ...
                while (Pp) {
                    ...
                    while (BP) {
                        ...
                        if (myrank == cg->rank) {
                            ...
                            else f_compute_rhs_bssn(RHS_PARA_CALLED_Constraint_Out); // deprecipated
                        }
...
void bssn_class::Interp_Constraint(bool infg)
{
    ...
    //    interpolate
    const int n = 1000;
    double lmax = GH->bbox[0][0][4];
    double lmin = 0;
    double dd = (lmax - lmin) / n;
    ...
    cudaStream_t stream = GPUManager::getInstance().get_stream();

    // 1. 点坐标传入 GPU
    double *d_pox[3];
    d_pox[0] = GPUManager::getInstance().allocate_device_memory(n);
    d_pox[1] = GPUManager::getInstance().allocate_device_memory(n);
    d_pox[2] = GPUManager::getInstance().allocate_device_memory(n);
    GPUManager::getInstance().sync_to_gpu(x1, d_pox[0], n);
    GPUManager::getInstance().sync_to_gpu(y1, d_pox[1], n);
    GPUManager::getInstance().sync_to_gpu(z1, d_pox[2], n);

    // 2. 初始化 Final Buffer
    double *d_final_shellf = GPUManager::getInstance().allocate_device_memory(n * InList);
    int *d_final_weight; cudaMalloc(&d_final_weight, n * sizeof(int));
    cudaMemset(d_final_shellf, 0, n * InList * sizeof(double));
    cudaMemset(d_final_weight, 0, n * sizeof(int));

    // 3. 从粗到细，逐层插值覆盖
    for (int lev = 0; lev < GH->levels; lev++) {
        PatList_Interp_Overwrite_GPU(
            stream, GH->PatL[lev], ConstraintList, 
            n, d_pox, d_final_shellf, d_final_weight, Symmetry
        );
    }
    GPUManager::getInstance().synchronize_all();

    double *shellf = new double[n * InList];
    GPUManager::getInstance().sync_to_cpu(shellf, d_final_shellf, n * InList);

    if (myrank == 0) {
        int* h_final_weight = new int[n];
        cudaMemcpy(h_final_weight, d_final_weight, n * sizeof(int), cudaMemcpyDeviceToHost);
        for(int i=0; i<n; i++) {
            if (h_final_weight[i] == 0) {
                cout << "WARNING: Point " << i << " (y=" << y1[i] << ") out of simulation domain!" << endl;
            }
        }
        delete[] h_final_weight;
    ...
    delete[] x1; delete[] y1; delete[] z1; delete[] shellf;
    GPUManager::getInstance().free_device_memory(d_pox[0], n);
    GPUManager::getInstance().free_device_memory(d_pox[1], n);
    GPUManager::getInstance().free_device_memory(d_pox[2], n);
    GPUManager::getInstance().free_device_memory(d_final_shellf, n * InList);
    cudaFree(d_final_weight);
}
...
void bssn_class::Compute_Constraint()
{
    ...
    for (lev = GH->levels - 1; lev > 0; lev--) RestrictProlong(lev, 1, false, ConstraintList, ConstraintList, ConstraintList);
}
