...
__global__ void bssn_core_rhs_part1_kernel(
    ...
    double* __restrict__ chi_rhs, double* __restrict__ gxx_rhs, double* __restrict__ gxy_rhs, double* __restrict__ gxz_rhs,
    ...
    double* __restrict__ Lap_rhs, double* __restrict__ betax_rhs, double* __restrict__ betay_rhs, double* __restrict__ betaz_rhs,
    ...
    double inv_chin1 = ONE / chin1; // 提取除法
    ...
    double inv_gupzz = ONE / (l_gxx * l_gyy * l_gzz + l_gxy * l_gyz * l_gxz + l_gxz * l_gxy * l_gyz - l_gxz * l_gyy * l_gxz - l_gxy * l_gxy * l_gzz - l_gxx * l_gyz * l_gyz);
    double gupxx = (l_gyy * l_gzz - l_gyz * l_gyz) * inv_gupzz;
    double gupxy = -(l_gxy * l_gzz - l_gyz * l_gxz) * inv_gupzz;
    double gupxz = (l_gxy * l_gyz - l_gyy * l_gxz) * inv_gupzz;
    double gupyy = (l_gxx * l_gzz - l_gxz * l_gxz) * inv_gupzz;
    double gupyz = -(l_gxx * l_gyz - l_gxy * l_gxz) * inv_gupzz;
    double gupzz = (l_gxx * l_gyy - l_gxy * l_gxy) * inv_gupzz;
    ...
    double A_contract_xx = gupxx * gupxx * l_Axx + gupxy * gupxy * l_Ayy + gupxz * gupxz * l_Azz + TWO*(gupxx * gupxy * l_Axy + gupxx * gupxz * l_Axz + gupxy * gupxz * l_Ayz);
    double A_contract_yy = gupxy * gupxy * l_Axx + gupyy * gupyy * l_Ayy + gupyz * gupyz * l_Azz + TWO*(gupxy * gupyy * l_Axy + gupxy * gupyz * l_Axz + gupyy * gupyz * l_Ayz);
    double A_contract_zz = gupxz * gupxz * l_Axx + gupyz * gupyz * l_Ayy + gupzz * gupzz * l_Azz + TWO*(gupxz * gupyz * l_Axy + gupxz * gupzz * l_Axz + gupyz * gupzz * l_Ayz);
    double A_contract_xy = gupxx * gupxy * l_Axx + gupxy * gupyy * l_Ayy + gupxz * gupyz * l_Azz + (gupxx * gupyy + gupxy * gupxy)* l_Axy + (gupxx * gupyz + gupxz * gupxy)* l_Axz + (gupxy * gupyz + gupxz * gupyy)* l_Ayz;
    double A_contract_xz = gupxx * gupxz * l_Axx + gupxy * gupyz * l_Ayy + gupxz * gupzz * l_Azz + (gupxx * gupyz + gupxy * gupxz)* l_Axy + (gupxx * gupzz + gupxz * gupxz)* l_Axz + (gupxy * gupzz + gupxz * gupyz)* l_Ayz;
    double A_contract_yz = gupxy * gupxz * l_Axx + gupyy * gupyz * l_Ayy + gupyz * gupzz * l_Azz + (gupxy * gupyz + gupyy * gupxz)* l_Axy + (gupxy * gupzz + gupyz * gupxz)* l_Axz + (gupyy * gupzz + gupyz * gupyz)* l_Ayz;

    double Lapx, Lapy, Lapz, Kx, Ky, Kz, chix, chiy, chiz;
    ...
    // 暂存计算 val_Gamx_rhs (将在 Part 2 完成剩余更新)
    double val_Gamx_rhs = - TWO * (Lapx * A_contract_xx + Lapy * A_contract_xy + Lapz * A_contract_xz) + 
        TWO * alpn1 * (-F3o2 * inv_chin1 * (chix * A_contract_xx + chiy * A_contract_xy + chiz * A_contract_xz) - gupxx * (F2o3 * Kx + EIGHT * PI * val_Sx) - gupxy * (F2o3 * Ky + EIGHT * PI * val_Sy) - gupxz * (F2o3 * Kz + EIGHT * PI * val_Sz) + 
        l_Gamxxx * A_contract_xx + l_Gamxyy * A_contract_yy + l_Gamxzz * A_contract_zz + TWO * (l_Gamxxy * A_contract_xy + l_Gamxxz * A_contract_xz + l_Gamxyz * A_contract_yz));

    double val_Gamy_rhs = - TWO * (Lapx * A_contract_xy + Lapy * A_contract_yy + Lapz * A_contract_yz) + 
        TWO * alpn1 * (-F3o2 * inv_chin1 * (chix * A_contract_xy + chiy * A_contract_yy + chiz * A_contract_yz) - gupxy * (F2o3 * Kx + EIGHT * PI * val_Sx) - gupyy * (F2o3 * Ky + EIGHT * PI * val_Sy) - gupyz * (F2o3 * Kz + EIGHT * PI * val_Sz) + 
        l_Gamyxx * A_contract_xx + l_Gamyyy * A_contract_yy + l_Gamyzz * A_contract_zz + TWO * (l_Gamyxy * A_contract_xy + l_Gamyxz * A_contract_xz + l_Gamyyz * A_contract_yz));

    double val_Gamz_rhs = - TWO * (Lapx * A_contract_xz + Lapy * A_contract_yz + Lapz * A_contract_zz) + 
        TWO * alpn1 * (-F3o2 * inv_chin1 * (chix * A_contract_xz + chiy * A_contract_yz + chiz * A_contract_zz) - gupxz * (F2o3 * Kx + EIGHT * PI * val_Sx) - gupyz * (F2o3 * Ky + EIGHT * PI * val_Sy) - gupzz * (F2o3 * Kz + EIGHT * PI * val_Sz) + 
        l_Gamzxx * A_contract_xx + l_Gamzyy * A_contract_yy + l_Gamzzz * A_contract_zz + TWO * (l_Gamzxy * A_contract_xy + l_Gamzxz * A_contract_xz + l_Gamzyz * A_contract_yz));
    ...
    double fxx = bx_gxxx + by_gxyy + bz_gxzz;
    double fxy = bx_gxyx + by_gyyy + bz_gyzz;
    double fxz = bx_gxzx + by_gyzy + bz_gzzz;
...
__global__ void bssn_core_rhs_part2_kernel(
    ...
    double* __restrict__ trK_rhs, double* __restrict__ Axx_rhs, double* __restrict__ Axy_rhs, double* __restrict__ Axz_rhs,
    ...
    // 读取并重构基础变量 (重新计算 gup** 以节省带宽)
    double val_chi = chi[idx];
    double chin1 = val_chi + ONE;
    double inv_chin1 = ONE / chin1;
    double alpn1 = Lap[idx] + ONE;
    double val_trK = trK[idx];
    ...
    // 从 Part1 提取原封不动的 Gam 和 Gam_rhs
    double l_Gamxxx = Gamxxx_out[idx]; double l_Gamxxy = Gamxxy_out[idx]; double l_Gamxxz = Gamxxz_out[idx];
    double l_Gamxyy = Gamxyy_out[idx]; double l_Gamxyz = Gamxyz_out[idx]; double l_Gamxzz = Gamxzz_out[idx];
    double l_Gamyxx = Gamyxx_out[idx]; double l_Gamyxy = Gamyxy_out[idx]; double l_Gamyxz = Gamyxz_out[idx];
    double l_Gamyyy = Gamyyy_out[idx]; double l_Gamyyz = Gamyyz_out[idx]; double l_Gamyzz = Gamyzz_out[idx];
    double l_Gamzxx = Gamzxx_out[idx]; double l_Gamzxy = Gamzxy_out[idx]; double l_Gamzxz = Gamzxz_out[idx];
    double l_Gamzyy = Gamzyy_out[idx]; double l_Gamzyz = Gamzyz_out[idx]; double l_Gamzzz = Gamzzz_out[idx];
    double val_Gamx_rhs = Gamx_rhs[idx]; double val_Gamy_rhs = Gamy_rhs[idx]; double val_Gamz_rhs = Gamz_rhs[idx];
    ...
    double gxxx = l_gxx * l_Gamxxx + l_gxy * l_Gamyxx + l_gxz * l_Gamzxx;
    double gxyx = l_gxx * l_Gamxxy + l_gxy * l_Gamyxy + l_gxz * l_Gamzxy;
    double gxzx = l_gxx * l_Gamxxz + l_gxy * l_Gamyxz + l_gxz * l_Gamzxz;
    double gyyx = l_gxx * l_Gamxyy + l_gxy * l_Gamyyy + l_gxz * l_Gamzyy;
    double gyzx = l_gxx * l_Gamxyz + l_gxy * l_Gamyyz + l_gxz * l_Gamzyz;
    double gzzx = l_gxx * l_Gamxzz + l_gxy * l_Gamyzz + l_gxz * l_Gamzzz;
    double gxxy = l_gxy * l_Gamxxx + l_gyy * l_Gamyxx + l_gyz * l_Gamzxx;
    double gxyy = l_gxy * l_Gamxxy + l_gyy * l_Gamyxy + l_gyz * l_Gamzxy;
    double gxzy = l_gxy * l_Gamxxz + l_gyy * l_Gamyxz + l_gyz * l_Gamzxz;
    double gyyy = l_gxy * l_Gamxyy + l_gyy * l_Gamyyy + l_gyz * l_Gamzyy;
    double gyzy = l_gxy * l_Gamxyz + l_gyy * l_Gamyyz + l_gyz * l_Gamzyz;
    double gzzy = l_gxy * l_Gamxzz + l_gyy * l_Gamyzz + l_gyz * l_Gamzzz;
    double gxxz = l_gxz * l_Gamxxx + l_gyz * l_Gamyxx + l_gzz * l_Gamzxx;
    double gxyz = l_gxz * l_Gamxxy + l_gyz * l_Gamyxy + l_gzz * l_Gamzxy;
    double gxzz = l_gxz * l_Gamxxz + l_gyz * l_Gamyxz + l_gzz * l_Gamzxz;
    double gyyz = l_gxz * l_Gamxyy + l_gyz * l_Gamyyy + l_gzz * l_Gamzyy;
    double gyzz = l_gxz * l_Gamxyz + l_gyz * l_Gamyyz + l_gzz * l_Gamzyz;
    double gzzz = l_gxz * l_Gamxzz + l_gyz * l_Gamyzz + l_gzz * l_Gamzzz;

    double fxx, fxy, fxz, fyy, fyz, fzz;
    double l_Rxx, l_Ryy, l_Rzz, l_Rxy, l_Rxz, l_Ryz;
    ...
    // 优化：提取公因式并减少乘法
    double term_xx = l_Axx*(gupxx*l_Axx + TWO*gupxy*l_Axy + TWO*gupxz*l_Axz) + l_Axy*(gupyy*l_Axy + TWO*gupyz*l_Axz) + gupzz*l_Axz*l_Axz;
    double term_yy = l_Axy*(gupxx*l_Axy + TWO*gupxz*l_Ayz) + l_Ayy*(gupyy*l_Ayy + TWO*gupxy*l_Axy + TWO*gupyz*l_Ayz) + gupzz*l_Ayz*l_Ayz;
    double term_zz = l_Axz*(gupxx*l_Axz + TWO*gupxy*l_Ayz) + l_Ayz*(gupyy*l_Ayz) + l_Azz*(gupzz*l_Azz + TWO*gupxz*l_Axz + TWO*gupyz*l_Ayz);
    ...
    // 重新获取 shift 的一阶导数以组装最终的 rhs (重算比传内存快得多)
    double betaxx, betaxy, betaxz, betayx, betayy, betayz, betazx, betazy, betazz;
    d_fderivs_point(dims, betax, &betaxx, &betaxy, &betaxz, X, Y, Z, ANTI, SYM, SYM, symmetry, lev, i, j, k);
    d_fderivs_point(dims, betay, &betayx, &betayy, &betayz, X, Y, Z, SYM, ANTI, SYM, symmetry, lev, i, j, k);
    d_fderivs_point(dims, betaz, &betazx, &betazy, &betazz, X, Y, Z, SYM, SYM, ANTI, symmetry, lev, i, j, k);
...
__global__ void bssn_constraints_kernel(
    ...
    double val_chi = chi[idx]; double chin1 = val_chi + ONE; double inv_chin1 = ONE / chin1;
    double val_trK = trK[idx];
    ...
    double inv_gupzz = ONE / gupzz;
    double gupxx = (l_gyy * l_gzz - l_gyz * l_gyz) * inv_gupzz;
    double gupxy = -(l_gxy * l_gzz - l_gyz * l_gxz) * inv_gupzz;
    double gupxz = (l_gxy * l_gyz - l_gyy * l_gxz) * inv_gupzz;
    double gupyy = (l_gxx * l_gzz - l_gxz * l_gxz) * inv_gupzz;
    double gupyz = -(l_gxx * l_gyz - l_gxy * l_gxz) * inv_gupzz;
    gupzz = (l_gxx * l_gyy - l_gxy * l_gxy) * inv_gupzz;
...
void gpu_compute_rhs_bssn_launch_opt(
    ...
    // ==========================================
    // 核心 Fission: Part 1 (寄存器占用减半)
    // ==========================================
    bssn_core_rhs_part1_kernel<<<grid, block, 0, stream>>>(
        ...
        Sx, Sy, Sz,
        chi_rhs, gxx_rhs, gxy_rhs, gxz_rhs, gyy_rhs, gyz_rhs, gzz_rhs,
    ...
    // 核心 Fission: Part 2
    // ==========================================
    bssn_core_rhs_part2_kernel<<<grid, block, 0, stream>>>(
        ex[0], ex[1], ex[2], symmetry, lev, X, Y, Z,
        chi, trK, dxx, gxy, gxz, dyy, gyz, dzz,
        Axx, Axy, Axz, Ayy, Ayz, Azz,
        Gamx, Gamy, Gamz, Lap, betax, betay, betaz, dtSfx, dtSfy, dtSfz,
        rho, Sx, Sy, Sz, Sxx, Sxy, Sxz, Syy, Syz, Szz,
        trK_rhs, Axx_rhs, Axy_rhs, Axz_rhs, Ayy_rhs, Ayz_rhs, Azz_rhs,
        Gamx_rhs, Gamy_rhs, Gamz_rhs, dtSfx_rhs, dtSfy_rhs, dtSfz_rhs,
        Gamxxx, Gamxxy, Gamxxz, Gamxyy, Gamxyz, Gamxzz,
        Gamyxx, Gamyxy, Gamyxz, Gamyyy, Gamyyz, Gamyzz,
        Gamzxx, Gamzxy, Gamzxz, Gamzyy, Gamzyz, Gamzzz,
